...
基于 Single-fiber pullout test 获取库仑摩擦模型下的平均界面滑动摩擦应力：

$$\tau_0 = \frac{P_{\text{peak}}}{\pi \cdot d_f \cdot L_e} \quad [\text{MPa}]$$

### 2. Matrix Fracture Mechanics (基体断裂力学)

利用单边缺口梁 (SENB) 三点弯曲试验计算基体的应力强度因子与裂尖能量释放率：

$$K_m = \frac{P_{\max} \cdot S}{b \cdot d^{1.5}} \cdot F\left(\frac{a_0}{d}\right) \quad [\text{MPa}\cdot\text{m}^{0.5}]$$

$$J_{\text{tip}} = \frac{K_m^2}{E_m} \quad [\text{J/m}^2]$$

> 注：几何修正函数 $F(a_0/d)$ 基于 Gross-Srawley 方程进行无量纲化处理。

### 3. Fiber Bridging Energy (纤维桥接互补能)

Complementary Energy ($J_b'$) 是决定多缝开裂 (Multiple Cracking) 的终极动力储备。通过对 σ-δ 曲线的峰值左侧区间应用 Simpson 法则进行数值积分获取：

$$J_b' = \sigma_0 \delta_0 - \int_{0}^{\delta_0} \sigma(\delta) \, \mathrm{d}\delta \quad [\text{J/m}^2]$$
...
$$\text{PSH}_{\text{strength}} = \frac{\sigma_0}{\sigma_{fc}} \geq 1.3$$

**Energy Criterion (维持稳态扩展):**

$$\text{PSH}_{\text{energy}} = \frac{J_b'}{J_{\text{tip}}} \geq 2.7$$

### 5. Theoretical Curve Simulation (纯理论 σ-δ 曲线预测)

宏观桥接应力由单根纤维的非线性拔出力经过双重积分叠加而成：
...
> 引擎内部包含了严苛的 Fiber Rupture 截断校验：$P \leq \frac{\pi d_f^2}{4}\sigma_{fu}$

### 6. Multi-Fiber Constitutive Extensions

**PVA Fibers (化学键模型):** 引入化学剥离位移界限：

$$\delta_d = \sqrt{\frac{G_d E_f A_f}{\pi d_f \tau_0^2}}$$

**Steel Fibers (端钩锚固模型):**

$$P_{\text{hook}}(\delta) = P_{h0} \left(1 - \frac{\delta}{\delta_{\text{hook}}}\right) \quad (\text{if } \delta < \delta_{\text{hook}})$$
...
1. **Input Material Parameters**: 输入纤维和基体参数
2. **Run Simulation**: 执行微观力学计算
3. **Analyze Results**: 查看 σ-δ 曲线和 PSH 评估
4. **Export Data**: 导出高质量图形和数据
